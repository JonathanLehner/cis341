...
(* compilation strategy 1: use the stack-based ir---------------------------- *)
...
(* compilation strategy 2: use a different invariant ------------------------ *)

(* Invariant: compile the expression directly into rax
...
let rec compile_exp (e:exp) : X86.ins list =
  ...
  begin match e with
    | Var x   -> Asm.[Movq, [compile_var x; ~%Rax]]
    | Const c -> Asm.[Movq, [Imm (Lit c); ~%Rax]]
